package ltd.finelink.read.ui.book.read.page.provider

import ltd.finelink.read.R
import ltd.finelink.read.model.ReadBook
import ltd.finelink.read.ui.book.read.page.api.DataSource
import ltd.finelink.read.ui.book.read.page.api.PageFactory
...
class TextPageFactory(dataSource: DataSource) : PageFactory<TextPage>(dataSource) {
    ...
    override fun moveToLast() = with(dataSource) {
        currentChapter?.let {
            if (it.pageSize == 0) {
                ltd.finelink.read.model.ReadBook.setPageIndex(0)
            } else {
                ltd.finelink.read.model.ReadBook.setPageIndex(it.pageSize.minus(1))
            }
        } ?: ltd.finelink.read.model.ReadBook.setPageIndex(0)
    }
    ...
    override fun moveToNext(upContent: Boolean): Boolean = with(dataSource) {
        return if (hasNext()) {
            ...
            if (currentChapter == null || currentChapter?.isLastIndex(pageIndex) == true) {
                ...
                ltd.finelink.read.model.ReadBook.moveToNextChapter(upContent, false)
            } else {
                if (pageIndex < 0 || currentChapter?.isLastIndexCurrent(pageIndex) == true) {
                    return@with false
                }
                ltd.finelink.read.model.ReadBook.setPageIndex(pageIndex.plus(1))
            }
    ...
    override fun moveToPrev(upContent: Boolean): Boolean = with(dataSource) {
        return if (hasPrev()) {
            if (pageIndex <= 0) {
                ...
                ltd.finelink.read.model.ReadBook.moveToPrevChapter(upContent, upContentInPlace = false)
            } else {
                if (currentChapter == null) {
                    return@with false
                }
                ltd.finelink.read.model.ReadBook.setPageIndex(pageIndex.minus(1))
            }
            if (upContent) upContent(resetPageOffset = false)
            true
        } else
    ...
    override val curPage: TextPage
        get() = with(dataSource) {
            ltd.finelink.read.model.ReadBook.msg?.let {
