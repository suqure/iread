...
import ltd.finelink.read.constant.*
import ltd.finelink.read.data.appDb
import ltd.finelink.read.data.entities.BaseBook
...
import ltd.finelink.read.utils.exists
...
val Book.isAudio: Boolean
    get() = isType(ltd.finelink.read.constant.BookType.audio)

val Book.isImage: Boolean
    get() = isType(ltd.finelink.read.constant.BookType.image)

val Book.isLocal: Boolean
    get() {
        if (type == 0) {
            return origin == ltd.finelink.read.constant.BookType.localTag || origin.startsWith(ltd.finelink.read.constant.BookType.webDavTag)
        }
        return isType(ltd.finelink.read.constant.BookType.local)
    }
...
val Book.isOnLineTxt: Boolean
    get() = !isLocal && isType(ltd.finelink.read.constant.BookType.text)

val Book.isWebFile: Boolean
    get() = isType(ltd.finelink.read.constant.BookType.webFile)

val Book.isUpError: Boolean
    get() = isType(ltd.finelink.read.constant.BookType.updateError)

val Book.isArchive: Boolean
    get() = isType(ltd.finelink.read.constant.BookType.archive)
...
fun Book.getRemoteUrl(): String? {
    if (origin.startsWith(ltd.finelink.read.constant.BookType.webDavTag)) {
        return origin.substring(ltd.finelink.read.constant.BookType.webDavTag.length)
    }
    return null
}
...
fun Book.upType() {
    if (type < 8) {
        type = when (type) {
            BookSourceType.image -> ltd.finelink.read.constant.BookType.image
            BookSourceType.audio -> ltd.finelink.read.constant.BookType.audio
            BookSourceType.file -> ltd.finelink.read.constant.BookType.webFile
            else -> ltd.finelink.read.constant.BookType.text
        }
        if (origin == ltd.finelink.read.constant.BookType.localTag || origin.startsWith(ltd.finelink.read.constant.BookType.webDavTag)) {
            type = type or ltd.finelink.read.constant.BookType.local
        }
    }
}
...
fun BookSource.getBookType(): Int {
    return when (bookSourceType) {
        ltd.finelink.read.constant.BookSourceType.file -> ltd.finelink.read.constant.BookType.text or ltd.finelink.read.constant.BookType.webFile
        ltd.finelink.read.constant.BookSourceType.image -> ltd.finelink.read.constant.BookType.image
        ltd.finelink.read.constant.BookSourceType.audio -> ltd.finelink.read.constant.BookType.audio
        else -> ltd.finelink.read.constant.BookType.text
    }
}
